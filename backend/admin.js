...
// ============ UTILIDADES INTERNAS PARA CSRF Y AUTH ============
...
/**
 * Genera los headers necesarios incluyendo el token CSRF y Authorization
 * Esto resuelve el error "CSRF token missing" al crear usuarios o productos.
 */
function getHeaders() {
    const headers = {
    ...
    // 1. Inyectar Token CSRF (necesario para seguridad Flask)
    const csrfToken = getCookie('csrf_token') || localStorage.getItem('csrf_token');
    if (csrfToken) {
        headers['X-CSRF-Token'] = csrfToken;
    }
    
    // 2. Inyectar Token de Autorización (necesario para validar tu ROL de manager)
...
function showSection(section) {
    ...
    // CORRECCIÓN: Validar que 'event' exista antes de usarlo para que no se congele el script
...
async function guardarProducto() {
    ...
    if (!data.nombre || !data.precio || !data.categoria_id) {
        mostrarNotificacion('Error', 'Nombre, precio y categoría son requeridos', 'danger');
        return;
    }
    ...
    try {
        ...
        if (response.ok && (result.success || result.producto_id || result.id)) {
            ...
            mostrarNotificacion('Guardado', `Producto procesado correctamente`, 'success');
            cargarProductosAdmin();
        } else {
...
async function guardarCategoria() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            mostrarNotificacion('Guardado', `Categoría procesada correctamente`, 'success');
            cargarCategorias();
        } else {
            const result = await response.json();
            mostrarNotificacion('Error', result.error || 'Error al guardar', 'danger');
...
async function eliminarCategoria(id) {
    ...
    try {
        ...
        if (response.ok) {
            mostrarNotificacion('Eliminada', 'Categoría eliminada', 'success');
            cargarCategorias();
        }
    } catch (error) {
...
} // <--- ESTA LLAVE FALTABA PARA CERRAR LA FUNCIÓN

function renderMateriaPrima() {
    const tbody = document.querySelector('#tabla-inventario tbody');
    if (!tbody) return;

    // Renderizado seguro usando el array global materiaPrima
    tbody.innerHTML = materiaPrima.map(item => {
        // 1. Lógica para determinar el estado del stock
...
function toggleCostoField() {
    ...
    const container = document.getElementById('ajuste-costo-container');
    if (container) container.style.display = tipo === 'entrada' ? 'block' : 'none';
}
...
async function realizarAjuste() {
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            mostrarNotificacion('Error', result.error || 'Error al procesar ajuste', 'danger');
        }
    } catch (error) {
...
async function guardarProveedor() {
    ...
    try {
        ...
        if (response.ok && (result.success || result.proveedor_id)) {
...
function mostrarModalOrden() {
    ...
    const modal = bootstrap.Modal.getOrCreateInstance(document.getElementById('modalOrden'));
    modal.show();
}
...
async function crearOrdenCompra() {
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            mostrarNotificacion('Orden Creada', `Orden ${result.numero} creada correctamente`, 'success');
            cargarOrdenes();
        } else {
            mostrarNotificacion('Error', result.error || 'Error al crear la orden', 'danger');
        }
    } catch (error) {
...
async function procesarRecepcion() {
    ...
    inputs.forEach(input => {
        ...
        if (cantidad > 0) {
            items.push({ item_id: input.dataset.itemId, cantidad_recibida: cantidad });
        }
    });
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            mostrarNotificacion('Recepción Procesada', `Recepción completada correctamente`, 'success');
            cargarOrdenes();
            cargarMateriaPrima();
            cargarEstadisticas();
        } else {
            mostrarNotificacion('Error', result.error || 'Error al procesar recepción', 'danger');
        }
    } catch (error) {
        console.error('Error:', error);
    }
...
function renderMovimientos(movimientos) {
    ...
    const tipoClases = { 'entrada': 'bg-success', 'salida': 'bg-danger', 'ajuste': 'bg-secondary' };
...
async function guardarUsuario() {
    ...
    try {
        ...
        if (response.ok && (result.success || result.usuario_id)) {
            ...
            mostrarNotificacion('Guardado', `Usuario procesado correctamente`, 'success');
            cargarUsuarios();
        } else {
            mostrarNotificacion('Error', result.error || 'Error al guardar', 'danger');
        }
...
async function resetearPassword() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            mostrarNotificacion('Éxito', 'Contraseña actualizada', 'success');
        } else {
            const result = await response.json();
            mostrarNotificacion('Error', result.error || 'Fallo al actualizar', 'danger');
        }
    } catch (error) {
...
window.mostrarResetPassword = (id, nombre) => {
            ...
            <td>
                <div class="btn-group btn-group-sm" role="group">
                    ...
                    ${cliente.credito_autorizado > 0 ? `
                        <button class="btn btn-outline-warning" onclick="verCreditoCliente(${cliente.id})" title="Ver crédito">
...
function mostrarModalCliente() {
    ...
    const fields = ['cliente-documento', 'cliente-nrc', 'cliente-nombre', 'cliente-nombre-comercial', 
                  'cliente-telefono', 'cliente-email', 'cliente-direccion', 'cliente-departamento', 
                  'cliente-municipio', 'cliente-actividad', 'cliente-notas'];
...
async function guardarCliente() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            mostrarNotificacion('Guardado', 'Cliente procesado correctamente', 'success');
            cargarClientes();
            cargarEstadisticasClientes();
        } else {
            const result = await response.json();
            mostrarNotificacion('Error', result.error || 'Fallo al guardar', 'danger');
        }
    } catch (error) {
...
async function eliminarCliente(id) {
    ...
    try {
        ...
        if (response.ok) {
            mostrarNotificacion('Eliminado', 'Cliente eliminado', 'success');
            cargarClientes();
            cargarEstadisticasClientes();
        }
    } catch (error) {
...
function toggleCamposContribuyente() {
    ...
    const isContribuyente = tipoCliente === 'contribuyente';
    
    ['campos-contribuyente', 'campo-nrc', 'campo-nombre-comercial'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.style.display = isContribuyente ? 'block' : 'none';
    });
}
            ...
            if (tablaEl) {
                tablaEl.innerHTML = data.pedidos.map(p => `
                    <tr>
                        ...
                        <td>${p.dte_numero_control ? `<code>${p.dte_numero_control}</code>` : '-'}</td>
                    </tr>
                `).join('');
            }
            bootstrap.Modal.getOrCreateInstance(document.getElementById('modalHistorialCliente')).show();
        }
    } catch (error) {
...
async function verCreditoCliente(id) {
    try {
        ...
        if (response.ok) {
            ...
            if (barraEl) {
                const porcentaje = data.credito_autorizado > 0 ? (data.credito_utilizado / data.credito_autorizado) * 100 : 0;
                barraEl.style.width = `${porcentaje}%`;
            }
            bootstrap.Modal.getOrCreateInstance(document.getElementById('modalCreditoCliente')).show();
        }
    } catch (error) {
...
// Exponer funciones finales globalmente
