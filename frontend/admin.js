...
let materiasprimasExtracciones = [];
let extraccionesActualesAdmin = [];

/**
 * Carga las materias primas para el selector de extracciones
 */
async function cargarMateriasPrimasExtracciones() {
    try {
        const response = await fetch(`${API_INVENTARIO}/materia-prima`);
        if (!response.ok) throw new Error('Error cargando materias primas');

        const data = await response.json();
        materiasprimasExtracciones = data.materias_primas || [];

        const select = document.getElementById('materiaPrimaIdAdmin');
        const currentValue = select.value;

        select.innerHTML = '<option value="">-- Seleccionar --</option>';
        materiasprimasExtracciones.forEach(mp => {
...
/**
 * Actualiza el preview del stock
 */
function actualizarPreviewStockAdmin() {
    const select = document.getElementById('materiaPrimaIdAdmin');
    const cantidad = parseFloat(document.getElementById('cantidadExtraidaAdmin').value) || 0;
    const unidadInput = document.getElementById('unidadMedidaAdmin');
    const previewDiv = document.getElementById('stockPreviewAdmin');
    ...
    document.getElementById('stockActualAdmin').textContent = `${stockActual.toFixed(2)} ${unidad}`;
    document.getElementById('aExtraerAdmin').textContent = `${cantidad.toFixed(2)} ${unidad}`;
    document.getElementById('stockDespuesAdmin').textContent = `${stockDespues.toFixed(2)} ${unidad}`;

    previewDiv.classList.remove('d-none');
}

/**
 * Registra una nueva extracción desde admin
 */
async function registrarExtraccionAdmin(event) {
    event.preventDefault();

    const fecha = document.getElementById('fechaFiltroExtracciones').value;
    const materiaPrimaId = document.getElementById('materiaPrimaIdAdmin').value;
    const cantidadExtraida = parseFloat(document.getElementById('cantidadExtraidaAdmin').value);
    const unidadMedida = document.getElementById('unidadMedidaAdmin').value;
    const motivo = document.getElementById('motivoAdmin').value;
    const descripcion = document.getElementById('descripcionAdmin').value;
    const user = JSON.parse(localStorage.getItem('user'));

    if (!fecha || !materiaPrimaId || !cantidadExtraida) {
        mostrarNotificacion('Validación', 'Por favor completa todos los campos requeridos', 'warning');
        return;
    }

    try {
        const response = await fetch(`${API_INVENTARIO}/extracciones`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (!response.ok) {
            mostrarNotificacion('Error', data.message || 'Error registrando extracción', 'danger');
            return;
        }

        mostrarNotificacion('Éxito', 'Extracción registrada correctamente', 'success');
        document.getElementById('formularioExtraccionAdmin').reset();
        document.getElementById('stockPreviewAdmin').classList.add('d-none');
        document.getElementById('unidadMedidaAdmin').value = '';

        await cargarMateriasPrimasExtracciones();
        await cargarExtraccionesAdmin();

    } catch (error) {
        console.error('Error:', error);
        mostrarNotificacion('Error', 'Error al registrar extracción', 'danger');
    }
}

/**
 * Carga las extracciones para la fecha seleccionada
 */
async function cargarExtraccionesAdmin() {
    try {
        const fecha = document.getElementById('fechaFiltroExtracciones').value;
        if (!fecha) {
            mostrarNotificacion('Validación', 'Por favor selecciona una fecha', 'warning');
            return;
        }

        const response = await fetch(`${API_INVENTARIO}/extracciones?fecha=${fecha}`, {
        ...
        const data = await response.json();
        extraccionesActualesAdmin = data.extracciones || [];

        renderizarTablaExtraccionesAdmin(extraccionesActualesAdmin);

    } catch (error) {
        console.error('Error:', error);
        document.getElementById('tablaExtraccionesContainer').innerHTML = `
            <div class="alert alert-danger">Error cargando extracciones</div>
        `;
    }
}

/**
 * Renderiza la tabla de extracciones
 */
function renderizarTablaExtraccionesAdmin(extracciones) {
    const container = document.getElementById('tablaExtraccionesContainer');

    if (extracciones.length === 0) {
        container.innerHTML = '<p class="text-muted text-center py-3">No hay extracciones registradas para esta fecha</p>';
        return;
    }
    ...
    extracciones.forEach(ext => {
        ...
        html += `
            <tr>
                ...
                <td>
                    <button class="btn btn-sm btn-danger" onclick="eliminarExtraccionAdmin(${ext.id})">
                        <i class="bi bi-trash"></i>
                    </button>
                </td>
            </tr>
 ...
 */
async function eliminarExtraccionAdmin(id) {
    if (!confirm('¿Eliminar esta extracción? El stock será revertido.')) return;

    try {
        const response = await fetch(`${API_INVENTARIO}/extracciones/${id}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${localStorage.getItem('auth_token')}` }
        });

        if (!response.ok) throw new Error('Error eliminando extracción');

        mostrarNotificacion('Éxito', 'Extracción eliminada y stock revertido', 'success');
        await cargarMateriasPrimasExtracciones();
        await cargarExtraccionesAdmin();

    } catch (error) {
        console.error('Error:', error);
        mostrarNotificacion('Error', 'Error al eliminar extracción', 'danger');
    }
}

/**
 * Inicializa las extracciones cuando se carga la sección
 */
function inicializarExtracciones() {
    const hoy = new Date().toISOString().split('T')[0];
    document.getElementById('fechaFiltroExtracciones').value = hoy;
    cargarMateriasPrimasExtracciones();
    cargarExtraccionesAdmin();
}
...
// Exponer funciones de extracciones globalmente
window.cargarExtraccionesAdmin = cargarExtraccionesAdmin;
window.registrarExtraccionAdmin = registrarExtraccionAdmin;
window.actualizarPreviewStockAdmin = actualizarPreviewStockAdmin;
window.eliminarExtraccionAdmin = eliminarExtraccionAdmin;
window.inicializarExtracciones = inicializarExtracciones;
